...
public:
	...
	void AcceptCommand(const BinaryOutput& aBo, size_t, int aSequence, IResponseAcceptor* apRspAcceptor, bool aDirectOperate = false) {
		mBinaryOutputs.push(aBo);
		this->AcceptCommand(aSequence, apRspAcceptor);
	}

	void AcceptCommand(const Setpoint& aSt, size_t, int aSequence, IResponseAcceptor* apRspAcceptor, bool aDirectOperate = false) {
		mSetpoints.push(aSt);
		this->AcceptCommand(aSequence, apRspAcceptor);
	}
